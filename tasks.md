...
> 文件版本：v1.1 | 對應 SPEC.md v0.3 + architecture.md v1.0 | 最後更新：2026-06-13
> 工作量標記：**S** = < 4h · **M** = 半天~1 天 · **L** = > 1 天
> DoD = Definition of Done（斜體）

## 進度摘要（v1.1, 2026-06-13）
...
- [x] **T0-1 定義 `report_lock.md` YAML schema**
...
- [x] **T0-2 撰寫 `shared-standards.md`（HTML/CSS 子集約束）**
...
- [x] **T1-1 實作 `config.py`（配置管理 + 字體 fail-fast）**
...
- [x] **T1-2 實作 `project_manager.py`（專案初始化）**
...
- [x] **T2-1 實作 `html_to_docx.py`（pandoc + reference docx）**
...
- [x] **T2-8 (wrapper; needs katex CLI at runtime) 實作 `katex_renderer.py`（katex-cli 預渲染 PNG）**
  - 掃描 `$$...$$` 與 `$...$`；呼叫 `katex-cli` 輸出 `assets/eq_N.png`；替換為 `<img>`
  - 預估：**M**
  - DoD：*含 3 個公式的 HTML 在 PDF 端正確顯示為 PNG；公式編號（Equation N）存在。*

- [x] **T2-9 整合 `report_gen.py` Phase 2 流程（PDF + DOCX 平行）**
...
- [x] **T3-17 撰寫 `SKILL.md`（主 workflow authority）**
...
*tasks.md v1.1 — 對應 SPEC.md v0.3 + architecture.md v1.0，2026-06-13*
*v1.1：標記 Phase 0+1 + 大部分 Phase 2 完成 (Tracks A+B 對應 T0-*+T1-*+大部分 T2-*)，Phase 3 workflows / CI / examples 累積待做*
